...
\begin{abstract}
	There have been remarkable advancements in the automobile industry the last century, safe, reliable and affordable vehicles is now a reality.
	Along with the progress in computation and communication we have been able to develop a real time operatin system (RTOS). RTOS makes it possible
	to implement differnet types of technologies in vehicles like RADAR, LIDAR, GPS, AI etc and made autonomous vehicles (AV) a reality. AV's purpose is
	not only to make it easier to travel, but safer as well. The end goal of AV's should always be to make it safer to travel on and along roads and
	city streets. We should not forget to think about ethical dilemmas when developing AV technology, but we should not be restricted by them.
	United Nations Economic Commission for Europe (UNECE) has this year (2020) made a regulation  that  will  allow  for  safer  introduction  for  AV’s
	in certain traffic situastions. They have made a regulation where there is given strict requirements for Automated Lane Keeping Systems (ALKS) and we are
	therfore now on level 3 in AV's. With this regulation makes me belive that the fully AV is not so far fetched in the near future.
...
of time- and safetycritical objectives.
 ...
 \subsection{Why do we use AI in vehicles?}
	...
	the list goes on. The reason we use AI in vehicles is to make the vehicle safer, not only for the driver but for everybody. %Føler at setningen er halv ferdig prøv gjerne å fullfør
...
\subsection{Implementation}
	% How is AI/ algorithms implemented in AV's
 ...
 \subsection{Ethics}
	...
	\subsubsection{Decisions}
		...
		with one person, should it hit and kill the one person or drive off the road potentially killing every one in the vehicle?
		This kind of extremal problem all though important to think about is from an engineering point of view unnecassery. Why not try avoid a situation
		like this all together? These kind of dilemmas maintains the assumption that the car only have two option when in reality it would have infinite.
		That the vehicle has such a high velocity it couldn't brake down to a speed that would'nt kill anyone is also unprobable in this scenario \cite{IEEE:ethics}.
		...
		We should not discard these kinds of problems all togehter. It is very important to have them in mind when developing AV technology.
		Vehicles should not be limited to binary choices, but rather multiple decisions based on optimal solutions from statistics.
		With these kinds of decisions and choices, we as a society have to consider how we want to distribute them.

	% Face recognition.
	% Recognize old from young?
	% How to decide safest outcome of given lifethreatening situation?
...
\section{Conclusion}
	...
	edge problem, trolley dilemmas is important to consider and should be discussed in public. It should not impede the progress regading development of AI fo AV's.
	City safe system currently in use in vehicles today, along with cutting edge break technology is allready generating safer environments for pedestrians than ever before.
	Even though we are at level 3AV's with the UNECE regulations for ALKS, the pilot of the vehicle still needs to supervice the driving.

	Our thoughts of the matter is that ethical challanges should be discussed alongside, and integrated into the development of future AV's. Not impede it.
	If all sorts of ethical dilemmas should be considered and evaluated equally, both in time and riskfactor, it is in our opinion that development will stagnate
	and be set back. The technologies in use in todays vehicles, both autonomous and non autonomous is still, in relative terms, new. And have not seen their full
	potential.
